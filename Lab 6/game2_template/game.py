...
def inventory_weight(inventory):
    ...
    for items in inventory:
        weight = weight + float(items["mass"])
    return weight
...
def execute_take(item_id):
    ...
    for item in items:
        if item["id"] == item_id:
            ...
            break
    if found:
        if (inventory_weight(inventory) + weight) <= 2.0:
            inventory.append(item)
            items.remove(item)

        else:
            print("\n" + "You're carrying too much weight, you cannot take that!")   
    else:
        print("\n" + "You cannot take that.")
